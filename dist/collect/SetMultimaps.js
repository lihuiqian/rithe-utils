...
function empty() {
    return new SetMultimap_1.default();
}
function from(iterable) {
    return new SetMultimap_1.default(iterable);
}
function invertFrom(map) {
    return new SetMultimap_1.default(Iterables_1.default.map(map, function (_a) {
...
function concat() {
    ...
    return multimaps.reduce(function (acc, multimap) { return (multimap.forEach(function (value, key) { return acc.set(key, value); }), acc); }, new SetMultimap_1.default());
}
function transform(multimap, project) {
    var result = new SetMultimap_1.default();
    multimap.forEach(function (value, key) { return result.set(key, project([key, value])); });
    return result;
}
function filter(multimap, predicate) {
    return new SetMultimap_1.default(Iterables_1.default.filter(multimap, predicate));
}
...
function elementsEqual(multimap1, multimap2) {
    ...
    multimap1.forEachCollection(function (collection, key) {
        ...
        if (!collection2 || !Sets_1.default.elementsEqual(collection, collection2))
            result = false;
    });
    return result;
}
function set(multimap) {
    ...
    var result = new SetMultimap_1.default(multimap);
...
//# sourceMappingURL=SetMultimaps.js.map